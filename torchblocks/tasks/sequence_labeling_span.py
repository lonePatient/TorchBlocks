...
class BertSpanForSeqLabel(BertPreTrainedModel):
    ...
    def decode(self, start_logits_or_labels, end_logits_or_labels, sequence_mask,
        ...
        if is_logits:  # 复用decode
            start_probs, start_labels = start_logits_or_labels.softmax(dim=-1).max(dim=-1)
            start_labels = torch.where(start_probs < start_thresh,
                                       torch.full_like(start_labels, other_id),
                                       start_labels)  # (batch_size, sequence_length)
            end_probs, end_labels = end_logits_or_labels.softmax(dim=-1).max(dim=-1)
            end_labels = torch.where(end_probs < end_thresh,
                                     torch.full_like(end_labels, other_id), end_labels)  # (batch_size, sequence_length)
        else:
        ...
        for i in range(batch_size):
            ...
            label_start_map = defaultdict(lambda: None)  # 每种类别设置起始标志，处理实体重叠情况，如：
            ...
            for pos, (s, e, m) in enumerate(zip(start_labels[i], end_labels[i], sequence_mask[i])):
                ...
                if m == 0: break
                if s != other_id:
                    label_start_map[s] = pos  # 以下两个功能：
                    # 1. 进入s类型span，以label_start_map[s]标记;
                    # 2. 若在s类型span内，但重新遇到s类型span起始时，更新位置
                if label_start_map[e] is not None:  # 在e类型span内（已包括处理单个token的实体）
                    decode_labels[-1].append((
                        label_start_map[e] - 1, pos,  # [CLS]
                        id2label[e]))  # 遇到结束位置，新建span
                    label_start_map[e] = None
