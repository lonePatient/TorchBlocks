...
class BertCrfForSeqLabel(BertPreTrainedModel, Application):
    ...
    def decode(self, logits, mask):
        decode_labels = self.crf.decode(logits, mask).squeeze(0)  # (batch_size, seq_length)
...
class SequenceLabelingTrainer(TrainBaseBuilder):
    ...
    def process_batch_outputs(self, outputs):
        preds, targets = [], []
        for logits, attention_mask, labels in zip(outputs['logits'], outputs['attention_mask'], outputs['labels']):
            tags = self.model.decode(logits.to(self.opts.device), attention_mask.to(self.opts.device))
            out_label_ids = tensor_to_list(labels)
            input_lens = tensor_to_list(torch.sum(attention_mask, dim=-1))
            tags = tensor_to_list(tags)
            for i, label in enumerate(out_label_ids):
                ...
                for j, m in enumerate(label):
                    if j == 0:
                        ...
                        preds.append(temp_2)
                        targets.append(temp_1)
                        break
                    else:
                        temp_1.append(self.opts.id2label[out_label_ids[i][j]])
                        temp_2.append(self.opts.id2label[tags[i][j]])
