...
class ToxicProcessor(TextClassifierProcessor):
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = " ".join(line['comment_text'])
            label_ids = [np.float(line[x]) for x in labels] if set_type != 'test' else None
            examples.append(
...
def main():
    ...
    processor = ToxicProcessor(tokenizer, args.data_dir, logger, prefix=prefix)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        results = {}
        eval_dataset = processor.create_dataset(max_seq_length=args.eval_max_seq_length,
                                                data_name='rvc1_test.json', mode='dev')
