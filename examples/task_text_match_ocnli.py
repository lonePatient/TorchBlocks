...
class OcnliDataset(DatasetBaseBuilder):
...
class ProcessEncodeText:
    ...
    def __call__(self, example):
        ...
        if example['label'] is not None:
            encoding["label"] = self.label2id[example["label"]]
        return encoding
...
def load_data(opts, file_name, data_type, tokenizer, max_sequence_length):
    process_piplines = [ProcessEncodeText(OcnliDataset.label2id(),
                                          tokenizer,
                                          tokenizer_params={
                                              "padding": "max_length",
    ...
    return OcnliDataset(opts, file_name, data_type=data_type, process_piplines=process_piplines)
...
def main():
    ...
    trainer = TextClassifierTrainer(opts=opts, model=model,
