...
class LcqmcProcessor(TextClassifierProcessor):
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[0]
            text_b = line[1]
            label = str(int(line[2])) if set_type != 'test' else None
            examples.append(
...
def main():
    ...
    processor = LcqmcProcessor(data_dir=args.data_dir, tokenizer=tokenizer, prefix=prefix)
    ...
    if args.do_train:
        ...
        eval_dataset = processor.create_dataset(args.eval_max_seq_length, 'dev.txt', 'dev')
    ...
    # do predict
    if args.do_predict:
        test_dataset = processor.create_dataset(args.eval_max_seq_length, 'test.txt', 'test')
