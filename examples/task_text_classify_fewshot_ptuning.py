...
# 定义数据集加载
class FewShotPtuningDataset(DatasetBaseBuilder):
    # zai collecate
...
class ProcessEncodeText:
    ...
    def __call__(self, example):
        ...
        num_extra_tokens = self.label_length + 2  # num_extra_tokens: <pseudo>,label,<pseudo>。
        ...
        # 构建pattern=sent1,<pseudo>,label,<pseudo>,sent2
...
# 定义任务的训练模块
class FewShotPtuningTrainer(TrainBaseBuilder):
    '''
    文本分类
    '''
    keys_to_ignore_on_gpu = ['raw_label', 'mask_span_indices']  # batch数据中不转换为GPU的变量名，比如'input_length’
    keys_to_ignore_on_save_result = ['input_ids', 'token_type_ids']  # eval和predict结果不存储的变量
    keys_to_ignore_on_save_checkpoint = ["optimzier"]  # checkpoint中不存储的模块，比如'optimizer'
...
def load_data(opts, file_name, data_type, tokenizer, max_sequence_length):
    ...
    return FewShotPtuningDataset(opts, file_name, data_type, process_piplines)
...
def main():
    ...
    labels = FewShotPtuningDataset.get_labels()
    label_desc = FewShotPtuningDataset.get_label_desc()
    ...
    metric = Accuracy(task="multiclass", num_classes=opts.num_labels)
    trainer = FewShotPtuningTrainer(opts=opts, model=model, metrics=[metric], logger=logger)
    # do train
    if opts.do_train:
