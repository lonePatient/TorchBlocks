...
from torchblocks.optims.adan import Adan
...
from torchblocks.optims.lr_scheduler import get_polynomial_decay_schedule_with_warmup
...
class TextClassifierTrainer(TrainBaseBuilder):
    ...
    def build_optimizer(self, model):
        self.logger.info("The custom optimizer is  `Adan` optimizer")
        optimizer_grouped_parameters = self.build_model_param_optimizer(model)
        optimizer = Adan(params=optimizer_grouped_parameters,
                         lr=self.opts.learning_rate,
                         eps=self.opts.adan_epsilon,
                         betas=(self.opts.adan_beta1, self.opts.adan_beta2, self.opts.adan_beta3),
    ...
    def build_lr_scheduler(self):
        ...
        scheduler = get_polynomial_decay_schedule_with_warmup(optimizer=self.optimizer,
...
def main():
    ...
    group = parser.add_argument_group(title="Adan optimizer", description="Adan pointer")
    group.add_argument("--adan_beta1", default=0.9, type=float, help="Beta1 for optimizer")
    group.add_argument("--adan_beta2", default=0.999, type=float, help='Beta2 for optimizer')
    group.add_argument("--adan_beta3", default=0.999, type=float, help='Beta2 for optimizer')
    group.add_argument('--adan_epsilon', default=1e-08, type=float)
