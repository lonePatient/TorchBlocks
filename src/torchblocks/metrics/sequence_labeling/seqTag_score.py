...
class SequenceLabelingScore(Metric):
    ...
    def value(self):
        ...
        recall = df[df['label'] == self.average]['recall'].item()
        precision = df[df['label'] == self.average]['precision'].item()
        return {
            "df": df, f"f1_{self.average}": f1, 'precision': precision, 'recall': recall  # for monitor
        }
