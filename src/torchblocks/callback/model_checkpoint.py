...
import logging
import numpy as np
from ..utils.ckpt_utils import save_model
from ..utils.io_utils import json_to_text
...
class ModelCheckpoint(object):
    ...
    def __init__(self,
        ...
        self.logger = tmp_logger if logger is None else logger
    ...
    def init_save_dir(self):
        ...
        if self.save_best:
            ...
            self.save_ckpt_dir = os.path.join(self.ckpt_dir, prefix + '-{:.2f}-step-{}')
    ...
    def _save_model(self, state):
        ...
        if self.verbose:
            self.logger.info("Saving model checkpoint to %s", state['save_dir'])
        model = state['model']
